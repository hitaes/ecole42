...
Form::Form(int sign_grade, int exec_grade): _name("default"), _is_signed(false), _grade_for_sign(sign_grade), _grade_for_exec(exec_grade)
{
	...
	std::endl;
	const int gfs = this->getGradeForSign();
	const int gfe = this->getGradeForExec();
	if (gfs > 150 || gfe > 150)
...
void Form::beSigned(Bureaucrat &signer)
{
	...
	else if (this->makeSign() == "not signed")
	{
...
const std::string	Form::makeSign(void)const
{
	if (this->_is_signed)
		return ("signed");
	else
